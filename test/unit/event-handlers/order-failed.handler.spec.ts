...
import { OrderFailedHandler } from '../../../src/infrastructure/event-handlers/order-failed.handler';
...
describe('OrderFailedHandler', () => {
  let handler: OrderFailedHandler;
  ...
  const mockOrderFailedEvent = new OrderFailedEvent(
    '123',
    '456',
    [
  ...
  beforeEach(async () => {
    ...
    handler = module.get<OrderFailedHandler>(OrderFailedHandler);
  ...
  describe('handle', () => {
    it('주문 실패 이벤트를 처리할 수 있어야 한다', async () => {
      ...
      await handler.handle(mockOrderFailedEvent);
    ...
    it('쿠폰을 사용하지 않은 경우 쿠폰 복구를 건너뛰어야 한다', async () => {
      const eventWithoutCoupon = new OrderFailedEvent(
        '123',
        '456',
        [{ productId: 1, quantity: 2, price: 10000 }],
        null, // couponId가 null
    ...
    it('보상 트랜잭션 실패 시 예외를 전파해야 한다', async () => {
      ...
      await expect(handler.handle(mockOrderFailedEvent)).rejects.toThrow('상품 조회 실패');
    });
  });
