...
export class OrderFailedHandler {
  ...
  /**
   * 주문 실패 이벤트 처리
   * 재고 및 쿠폰 복구 보상 트랜잭션 수행
   */
  async handle(event: OrderFailedEvent): Promise<void> {
    try {
      this.logger.log(`🎯 주문 실패 이벤트 처리 시작: 주문 ${event.orderId}`);
    ...
    this.logger.log(`✅ 주문 실패 이벤트 처리 완료: 주문 ${event.orderId}, 보상 트랜잭션 완료`);
    } catch (error) {
      this.logger.error(`❌ 주문 실패 이벤트 처리 실패: 주문 ${event.orderId}`, error);
      // 보상 트랜잭션 실패는 심각한 문제이므로 로깅 후 예외 전파
      throw error;
    }
  }
